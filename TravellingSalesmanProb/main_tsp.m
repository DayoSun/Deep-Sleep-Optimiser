%{
TSP
Stephen Ekwe and Dayo Oladejo 22nd September 2022
2:44am
%}
global matriz
%% Select file
...
matriz = distance(cities);

%% determine the distance

% matriz=[ 0 107 241 190 124  80 316  76 152 157 283 133 113 297 228 129 348 276 188 150  65 341 184  67 221 169 108  45 167
...
%xlswrite('filename.xlsx',matriz)


%%

%AA1 = [0 7.071 10 7.141 5.831; 7.071 0 7.071 30.02 8.0; 10 7.071 0 35.23 5.83; 7.141 30.02 35.23 0 38.01; 5.831 8 5.83 38.01 0];
%matriz =AA1;
%% TSP problem https://developers.google.com/optimization/routing/tsp
% Optimal answer is 7293
%  1. New York - 2. Los Angeles - 3. Chicago - 4. Minneapolis - 5. Denver - 6. Dallas
%- 7. Seattle - 8. Boston - 9. San Francisco - 10. St. Louis - 11. Houston - 12. Phoenix - 13. Salt Lake City

% AA1 = [0, 2451, 713, 1018, 1631, 1374, 2408, 213, 2571, 875, 1420, 2145, 1972;
...
%%% Parameters
Episode = 1;                % No. of Episodes
MaxIt = 100000;                % Maximum Iteration
nPop =10000;                  % No. of Agents
nVar = length(matriz);     % No. of variables
LB = 0*ones(1,nVar);        % Variable Bounds
UB = ones(1,nVar);       % Variable Bounds 10000000*
...
r_trn = 1:nVar;
plot(cities(1,:), cities(2,:), 'bs','MarkerFaceColor','g', 'MarkerSize',18  )
hold on;
plot(cities(1,path(1)),cities(2,path(1)), 'bs','MarkerFaceColor','r', 'MarkerSize',22)
hold on;
strValues = strtrim(cellstr(num2str(r_trn(:))));
text(cities(1,:), cities(2,:), strValues, 'HorizontalAlignment','center', 'VerticalAlignment','middle', 'fontsize',12);
plot(cities(1, path),cities(2, path))
%hold on
%plot(cities(1,path(1)),cities(2,path(1)), 'bs','MarkerFaceColor','g', 'MarkerSize',22)



% plot(cities(1,:), cities(2,:), 'bs','MarkerFaceColor','g' )
% hold on;
% plot(cities(1, path),cities(2, path))
